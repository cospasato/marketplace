  ...
  return (
    <div style={{ minHeight: "100vh" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", color: "var(--accent2)", marginBottom: 10, textTransform: "uppercase" }}>
          {registry.occasion} Registry
        </div>
        <h1 style={{ fontFamily: "Georgia, serif", fontSize: "clamp(28px, 5vw, 52px)", fontWeight: 800, color: "var(--text)", lineHeight: 1.1, marginBottom: 12 }}>
          {registry.title}
        </h1>
        <p style={{ fontSize: 15, color: "var(--text2)", marginBottom: 8 }}>Created by {registry.ownerName}</p>
        {registry.eventDate && (
          <div style={{ fontSize: 13, color: daysUntil > 0 ? "var(--accent2)" : "var(--text3)" }}>
            📅 {new Date(registry.eventDate).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })}
            {daysUntil > 0 && <span style={{ marginLeft: 8, color: "var(--accent2)" }}>— {daysUntil} days away</span>}
          </div>
        )}
      ...
      {topGifters.length > 0 && topGifters.some(g => g.total > 0) && (
        <div style={{ background: "var(--bg2)", borderBottom: "1px solid #1a1a1a", padding: "16px 24px" }}>
          <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: "var(--accent2)", letterSpacing: "0.08em", flexShrink: 0 }}>🏆 TOP GIFTERS</span>
            {topGifters.map((g, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--bg2)", border: `1px solid ${i === 0 ? "rgba(232,213,176,0.25)" : "#1e1e1e"}`, borderRadius: 100, padding: "6px 14px 6px 8px" }}>
                <div style={{ width: 26, height: 26, borderRadius: "50%", background: i === 0 ? "rgba(232,213,176,0.15)" : "var(--bg3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: i === 0 ? "var(--accent)" : "var(--text3)" }}>
                  {i === 0 ? "🥇" : i === 1 ? "🥈" : "🥉"}
                </div>
                <div>
                  <span style={{ fontSize: 13, color: i === 0 ? "var(--accent)" : "var(--text2)", fontWeight: i === 0 ? 700 : 400 }}>{g.name}</span>
                  {g.total > 0 && <span style={{ fontSize: 11, color: "var(--accent2)", marginLeft: 6 }}>· {registry.items?.[0]?.currency || "USD"} {g.total.toFixed(0)}</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      {registry.contributions.length > 0 && (
        <div style={{ background: "var(--bg2)", borderBottom: "1px solid #1a1a1a", padding: "16px 24px", overflowX: "auto" }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center", maxWidth: 1100, margin: "0 auto", minWidth: "max-content" }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: "var(--text3)", letterSpacing: "0.08em", flexShrink: 0 }}>GIFTERS</span>
            {registry.contributions.slice(0, 8).map((c, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 100, padding: "5px 14px 5px 6px" }}>
                <div style={{ width: 26, height: 26, borderRadius: "50%", background: "var(--bg3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: "var(--accent)" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
          <div style={{ fontFamily: "Georgia, serif", fontSize: 18, color: "var(--text)" }}>
            {registry.items.length} gifts
          </div>
          <div style={{ display: "flex", gap: 6, background: "var(--bg2)", padding: 4, borderRadius: 10 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
            {filteredItems.map(item => {
              ...
              return (
                ...
                onMouseEnter={e => isAvailable && (e.currentTarget.style.borderColor = "var(--bg5)")}
                ...
                >
                  ...
                  <div style={{ padding: "14px 16px" }}>
                    <div style={{ fontFamily: "Georgia, serif", fontSize: 15, fontWeight: 700, color: "var(--text)", marginBottom: 6, lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                      {item.title}
                    </div>
                    {item.note && <p style={{ fontSize: 12, color: "var(--text3)", marginBottom: 8, lineHeight: 1.5, fontStyle: "italic" }}>{item.note}</p>}
                    <div style={{ fontFamily: "Georgia, serif", fontSize: 18, fontWeight: 800, color: "var(--accent)", marginBottom: 14 }}>
                    ...
                    {isAvailable ? (
                      ...
                      <div style={{ width: "100%", padding: "10px", background: "var(--bg3)", borderRadius: 10, textAlign: "center", fontSize: 13, color: "var(--text3)" }}>
                        Already {item.status}
                      </div>
                    )}
                  </div>
      ...
      {registry.thankYouMsg && (
        <div style={{ background: "var(--bg2)", borderTop: "1px solid #1a1a1a", padding: "40px 24px", textAlign: "center" }}>
          <div style={{ maxWidth: 480, margin: "0 auto" }}>
            ...
            <p style={{ fontSize: 13, color: "var(--text3)", marginTop: 12 }}>— {registry.ownerName}</p>
          </div>
        </div>
      )}
      ...
      {claimModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={() => setClaimModal(null)}>
          <div style={{ background: "var(--bg2)", border: "1px solid var(--border2)", borderRadius: 20, padding: 28, maxWidth: 480, width: "100%", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            {!claimResult ? (
              <>
                <div style={{ display: "flex", gap: 14, marginBottom: 24 }}>
                  <div style={{ width: 72, height: 72, borderRadius: 12, overflow: "hidden", background: "var(--bg3)", flexShrink: 0 }}>
                    {claimModal.imageUrl && <img src={claimModal.imageUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />}
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: "var(--accent2)", marginBottom: 4, fontWeight: 700, letterSpacing: "0.08em" }}>YOU'RE GIFTING</div>
                    <div style={{ fontFamily: "Georgia, serif", fontSize: 16, fontWeight: 700, color: "var(--text)", lineHeight: 1.3 }}>{claimModal.title}</div>
                    <div style={{ fontFamily: "Georgia, serif", fontSize: 18, fontWeight: 800, color: "var(--accent)", marginTop: 4 }}>{claimModal.currency} {claimModal.price.toFixed(2)}</div>
                  </div>
                </div>

                <p style={{ fontSize: 13, color: "var(--text2)", marginBottom: 20, lineHeight: 1.6, background: "var(--bg3)", padding: "12px 14px", borderRadius: 10 }}>
                  Claim this gift to let others know it's taken. You'll then be redirected to buy it directly from the store.
                </p>

                <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 20 }}>
                  <div>
                    <label style={{ display: "block", fontSize: 10, fontWeight: 700, color: "var(--text3)", marginBottom: 5, letterSpacing: "0.08em", textTransform: "uppercase" }}>Your name *</label>
                    <input value={claimForm.gifterName} onChange={e => setClaimForm(f => ({ ...f, gifterName: e.target.value }))} placeholder="Your name" style={inp} />
                  </div>
                  <div>
                    <label style={{ display: "block", fontSize: 10, fontWeight: 700, color: "var(--text3)", marginBottom: 5, letterSpacing: "0.08em", textTransform: "uppercase" }}>Your email *</label>
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: 10, fontWeight: 700, color: "var(--text3)", marginBottom: 5, letterSpacing: "0.08em", textTransform: "uppercase" }}>Message (optional)</label>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  <button onClick={handleClaim} disabled={claiming || !claimForm.gifterName || !claimForm.gifterEmail} style={{
                    flex: 1, padding: "13px", background: "var(--accent)", color: "var(--bg)",
                    borderRadius: 10, border: "none", fontFamily: "Georgia, serif",
                    fontWeight: 800, fontSize: 15, cursor: "pointer", opacity: claiming ? 0.7 : 1,
                  }}>
                    {claiming ? "Claiming..." : "Claim & Buy Gift →"}
                  </button>
                  <button onClick={() => setClaimModal(null)} style={{ padding: "13px 18px", background: "var(--bg3)", border: "1px solid var(--border2)", borderRadius: 10, color: "var(--text2)", cursor: "pointer" }}>Cancel</button>
                </div>
              </>
            ) : claimResult.error ? (
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                ...
                <button onClick={() => setClaimModal(null)} style={{ padding: "10px 24px", background: "var(--bg3)", border: "1px solid var(--border2)", borderRadius: 10, color: "var(--text2)", cursor: "pointer" }}>Close</button>
              </div>
            ) : (
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                ...
                <br />
                <button onClick={() => setClaimModal(null)} style={{ padding: "8px 20px", background: "transparent", border: "none", color: "var(--text3)", cursor: "pointer", fontSize: 13 }}>
                  Back to registry
                </button>
              </div>
            )}
