...
export default function RegistryDashboard() {
  ...
  if (!registry) return (
    <div style={{ padding: 80, textAlign: "center", color: "#f87171" }}>
      <p style={{ marginBottom: 16 }}>Registry not found.</p>
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 32, gap: 16, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button onClick={copyLink} style={{
            padding: "9px 18px", borderRadius: 10, cursor: "pointer", fontFamily: "inherit", fontWeight: 600, fontSize: 13,
            background: copied ? "rgba(74,222,128,0.1)" : "#1a1a1a",
            border: `1px solid ${copied ? "#4ade80" : "#2a2a2a"}`,
            color: copied ? "#4ade80" : "#9a9690",
          }}>
      ...
      {/* Share box */}
      <div style={{ background: "rgba(232,213,176,0.05)", border: "1px solid rgba(232,213,176,0.15)", borderRadius: 14, padding: "18px 22px", marginBottom: 28, display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        <span style={{ fontSize: 22 }}>🔗</span>
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#c4a870", marginBottom: 4 }}>YOUR REGISTRY LINK</div>
          <div style={{ fontSize: 12, color: "#9a9690", fontFamily: "monospace", wordBreak: "break-all" }}>{shareLink}</div>
        </div>
        <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
          <a href={`https://wa.me/?text=Check%20my%20registry%3A%20${encodeURIComponent(shareLink)}`} target="_blank" rel="noopener noreferrer"
          ...
          <a href={`mailto:?subject=My%20Gift%20Registry&body=Hi!%20Check%20my%20registry%3A%20${encodeURIComponent(shareLink)}`}
            style={{ padding: "7px 14px", background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 8, color: "#9a9690", fontSize: 12, textDecoration: "none", fontWeight: 600 }}>
            Email
          </a>
        </div>
                ...
                <div key={item.id} style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, overflow: "hidden" }}>
                  <div style={{ position: "relative", aspectRatio: "16/9", background: "#1a1a1a" }}>
                    ...
                    <div style={{ position: "absolute", top: 10, right: 10, padding: "3px 10px", borderRadius: 100, fontSize: 10, fontWeight: 700,
                      background: `${STATUS_COLORS[item.status]}18`, color: STATUS_COLORS[item.status], border: `1px solid ${STATUS_COLORS[item.status]}30` }}>
                  ...
                  <div style={{ padding: "14px 16px" }}>
                    ...
                    <div style={{ fontFamily: "Georgia, serif", fontSize: 17, fontWeight: 800, color: "#e8d5b0", marginBottom: item.note ? 8 : 0 }}>
                ...
                return (
                  <div key={product.id} style={{ background: "#111", border: `1px solid ${inRegistry ? "#4ade8040" : "#1e1e1e"}`, borderRadius: 14, overflow: "hidden" }}>
                    <div style={{ aspectRatio: "1", background: "#1a1a1a", position: "relative" }}>
                      ...
                      {inRegistry && (
                        <div style={{ position: "absolute", inset: 0, background: "rgba(74,222,128,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28 }}>✓</div>
                      )}
                    </div>
                    <div style={{ padding: "12px 14px" }}>
                      ...
                      <button onClick={() => !inRegistry && addToRegistry(product)} disabled={inRegistry || addingProduct === product.id} style={{
                        width: "100%", padding: "8px", border: "none", borderRadius: 8,
                        cursor: inRegistry ? "default" : "pointer",
