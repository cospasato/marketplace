...
function useConfetti(canvasRef) {
  ...
  const burst = useCallback(() => {
    ...
    const colors = ["var(--gold)","var(--yellow)","var(--green)","var(--blue)","var(--red)","var(--gold-dk)","#fff","var(--purple)"];
    for (let i = 0; i < 150; i++) {
...
export default function LiveDashboard({ slug }) {
  ...
  if (!slug) return (
    <div style={{ minHeight: "100vh", background: "#000", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center", color: "var(--red)", padding: 40 }}>
        ...
        <p style={{ fontSize: 13, color: "#9a9690", marginTop: 8 }}>URL should be: /registry/live/your-registry-slug</p>
      </div>
    </div>
  );
  ...
  if (error && !registry) return (
    <div style={{ minHeight: "100vh", background: "#000", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center", color: "var(--red)", padding: 40, maxWidth: 400 }}>
        ...
        <p style={{ fontSize: 13, color: "#9a9690" }}>Slug used: <code style={{ color: "#c4a870" }}>{slug}</code></p>
        <button onClick={poll} style={{ marginTop: 20, padding: "10px 24px", background: "var(--gold)", color: "var(--white)", borderRadius: 8, border: "none", cursor: "pointer", fontWeight: 700 }}>Retry</button>
      </div>
    </div>
  );
  ...
  if (!registry) return (
    <div style={{ minHeight: "100vh", background: "#000", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
      ...
      <div style={{ color: "#9a9690", fontSize: 14 }}>Connecting to {slug}...</div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#000", color: "#f5f0e8", fontFamily: "Georgia, serif", overflow: "hidden", position: "relative", paddingBottom: 48 }}>
      ...
      {/* Top bar */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 28px", borderBottom: "1px solid var(--border)", background: "rgba(0,0,0,0.9)", backdropFilter: "blur(12px)", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 11, color: "#9a9690", fontFamily: "sans-serif", letterSpacing: "0.1em" }}>{registry.occasion?.toUpperCase()} REGISTRY</div>
          <div style={{ fontSize: 18, fontWeight: 700, color: "#e8d5b0" }}>{registry.title}</div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#f5f0e8", fontFamily: "sans-serif" }}>
            {clock.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}
          </div>
          <div style={{ fontSize: 11, color: "#9a9690", fontFamily: "sans-serif" }}>{clock.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}</div>
        </div>
      </div>
      ...
      <div style={{ padding: "24px 28px", display: "grid", gridTemplateColumns: "1fr 360px", gap: 20, maxWidth: 1400, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
            {[
              { label: "Total Gifts", value: total, icon: "🎁", color: "#f5f0e8" },
              ...
              <div key={label} style={{ background: "#0d0d0d", border: "1px solid #1a1a1a", borderRadius: 14, padding: "18px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 10, color: "#9a9690", fontFamily: "sans-serif", letterSpacing: "0.08em", textTransform: "uppercase" }}>{label}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: "#0d0d0d", border: "1px solid #1a1a1a", borderRadius: 14, padding: "20px 24px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, fontSize: 12, color: "#9a9690", fontFamily: "sans-serif" }}>
          ...
          <div style={{ background: "#0d0d0d", border: "1px solid #1a1a1a", borderRadius: 14, padding: "18px 20px" }}>
            <div style={{ fontSize: 11, color: "#9a9690", letterSpacing: "0.08em", marginBottom: 14, fontFamily: "sans-serif" }}>GIFT LIST</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 8 }}>
              {items.map(item => (
                <div key={item.id} style={{
                  background: item.status === "purchased" ? "rgba(74,222,128,0.08)" : item.status === "claimed" ? "rgba(245,158,11,0.08)" : "var(--off-white)",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {showCelebration && lastGift && (
            <div style={{ background: "linear-gradient(135deg, rgba(232,213,176,0.12), rgba(196,168,112,0.08))", border: "1px solid rgba(232,213,176,0.25)", borderRadius: 18, padding: "22px 18px", textAlign: "center", animation: "popIn 0.4s ease" }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 700, color: "#f5f0e8", marginTop: 4 }}>{lastGift.item?.title || "a gift"}</div>
          ...
          {topGifters.length > 0 && (
            <div style={{ background: "#0d0d0d", border: "1px solid rgba(196,168,112,0.15)", borderRadius: 16, padding: "18px 18px" }}>
              ...
              {topGifters.map((g, i) => (
                <div key={g.email} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i < topGifters.length - 1 ? "1px solid var(--border)" : "none" }}>
                  <div style={{ width: 28, height: 28, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 13,
                    background: i === 0 ? "rgba(232,213,176,0.2)" : i === 1 ? "rgba(180,180,180,0.15)" : i === 2 ? "rgba(180,120,60,0.15)" : "var(--cream)",
                    color: i === 0 ? "var(--gold)" : i === 1 ? "#c0c0c0" : i === 2 ? "#b47830" : "var(--gray)",
                    border: `1px solid ${i === 0 ? "rgba(232,213,176,0.3)" : i === 1 ? "rgba(180,180,180,0.2)" : i === 2 ? "rgba(180,120,60,0.2)" : "var(--gray-bg)"}`,
                  }}>
                    {i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : i + 1}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: i === 0 ? "var(--gold)" : "var(--black)", fontFamily: "sans-serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{g.name}</div>
                    <div style={{ fontSize: 11, color: "#9a9690", fontFamily: "sans-serif" }}>{g.count} gift{g.count !== 1 ? "s" : ""}</div>
                  </div>
                  <div style={{ fontSize: 15, fontWeight: 800, color: i === 0 ? "var(--gold)" : "var(--text2)", fontFamily: "sans-serif", flexShrink: 0 }}>
          ...
          <div style={{ background: "#0d0d0d", border: "1px solid #1a1a1a", borderRadius: 16, padding: "18px" }}>
            <div style={{ fontSize: 11, color: "#9a9690", letterSpacing: "0.08em", marginBottom: 12, fontFamily: "sans-serif" }}>GIFT FEED ({contributions.length})</div>
            {contributions.length === 0 ? (
              <div style={{ textAlign: "center", padding: "20px 0", color: "#9a9690", fontSize: 13, fontFamily: "sans-serif" }}>Waiting for first gift... 🎁</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 260, overflowY: "auto" }}>
                {contributions.slice(0, 20).map((c, i) => (
                  <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", background: i === 0 ? "rgba(232,213,176,0.06)" : "var(--off-white)", border: `1px solid ${i === 0 ? "rgba(232,213,176,0.12)" : "var(--cream)"}`, borderRadius: 10, animation: i === 0 ? "slideIn 0.4s ease" : "none" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "#f5f0e8", fontFamily: "sans-serif" }}>{c.gifterName}</div>
                      <div style={{ fontSize: 10, color: "#9a9690", fontFamily: "sans-serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          ...
          <div style={{ background: "#0d0d0d", border: "1px solid #1a1a1a", borderRadius: 16, padding: "18px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: "#f5f0e8" }}>{registry.ownerName}</div>
            ...
            <div style={{ fontSize: 11, color: "#9a9690", marginTop: 10, fontFamily: "monospace", wordBreak: "break-all" }}>
      ...
      {/* Ticker */}
      {contributions.length > 0 && (
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "rgba(0,0,0,0.95)", borderTop: "1px solid var(--border)", padding: "8px 0", overflow: "hidden", zIndex: 50 }}>
          <div style={{ display: "inline-flex", gap: 48, animation: "ticker 25s linear infinite", whiteSpace: "nowrap", fontSize: 13, color: "#9a9690", fontFamily: "sans-serif" }}>
            {[...contributions.slice(0, 10), ...contributions.slice(0, 10)].map((c, i) => (
              <span key={i} style={{ color: i % 2 === 0 ? "var(--gold-dk)" : "var(--text2)" }}>
