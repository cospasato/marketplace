...
const emptyStore = { shopDomain: "", storeName: "", storefrontToken: "", description: "", currency: "USD", primaryColor: "" };
...
export default function AdminPage() {
  ...
  const syncStore = async (id) => {
    ...
    setSyncing(null);
    flash(data.message || "Sync complete");
    await load();
  };
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32 }}>
        ...
        <button onClick={syncAll} disabled={syncing === "all"} style={{
          padding: "10px 22px", background: syncing === "all" ? "var(--bg4)" : "var(--accent)",
          color: syncing === "all" ? "var(--text3)" : "#0a0a0a",
          borderRadius: "var(--radius)", fontFamily: "var(--font-display)",
          fontWeight: 700, fontSize: 14, cursor: syncing === "all" ? "not-allowed" : "pointer",
          transition: "all 0.15s",
        }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 3 }}>
                      ...
                      <span style={{
                        ...
                        background: store.active ? "rgba(74,222,128,0.1)" : "rgba(248,113,113,0.1)",
                        color: store.active ? "var(--green)" : "var(--red)",
                        border: `1px solid ${store.active ? "rgba(74,222,128,0.2)" : "rgba(248,113,113,0.2)"}`,
                      }}>
                        {store.active ? "Active" : "Inactive"}
                      </span>
                      {store.featured && (
                        <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", padding: "2px 8px", borderRadius: 100, background: "rgba(196,168,112,0.1)", color: "var(--accent2)", border: "1px solid rgba(196,168,112,0.2)" }}>
                          Featured
                        </span>
                      )}
                    </div>
                    <div style={{ fontSize: 12, color: "var(--text3)", fontFamily: "var(--font-body)" }}>
                      {store.shopDomain} · {store.productCount} products · {store.currency}
                    </div>
                  </div>

                  <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                    <button onClick={() => syncStore(store.id)} disabled={syncing === store.id} style={{
                    ...
                    <button onClick={() => editStore(store)} style={{
                      padding: "7px 14px", borderRadius: "var(--radius)",
                      border: "1px solid var(--border2)", background: "transparent",
                      color: "var(--text2)", fontSize: 12, fontWeight: 600, cursor: "pointer",
                    }}>
      ...
      {tab === "add" && (
        <div style={{ maxWidth: 560 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {[
              { key: "shopDomain", label: "Shopify domain", placeholder: "your-store.myshopify.com", disabled: !!editing },
              { key: "storeName", label: "Store display name", placeholder: "My Awesome Store" },
              { key: "storefrontToken", label: "Storefront API token", placeholder: "shpat_xxxxxxxxxxxx" },
              { key: "description", label: "Short description (optional)", placeholder: "Fashion & accessories" },
              { key: "currency", label: "Currency", placeholder: "USD" },
              { key: "primaryColor", label: "Brand color (optional, hex)", placeholder: "#4a90e2" },
            ].map(({ key, label, placeholder, disabled }) => (
              <div key={key}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text3)", marginBottom: 6, letterSpacing: "0.06em", textTransform: "uppercase" }}>{label}</label>
                <input
                  value={form[key]}
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
              <button onClick={saveStore} disabled={saving} style={{
                flex: 1, padding: "13px", background: "var(--accent)", color: "#0a0a0a",
                borderRadius: "var(--radius)", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 15, cursor: "pointer",
              }}>
                {saving ? "Saving..." : editing ? "Save changes" : "Add store"}
              </button>
              {editing && (
                <button onClick={() => { setEditing(null); setForm(emptyStore); setTab("stores"); }} style={{
                  padding: "13px 20px", background: "var(--bg3)", border: "1px solid var(--border2)",
                  color: "var(--text2)", borderRadius: "var(--radius)", cursor: "pointer", fontSize: 14,
                }}>
                  Cancel
                </button>
              )}
            ...
            {!editing && (
              <div style={{ marginTop: 8, padding: 16, background: "var(--bg3)", border: "1px solid var(--border)", borderRadius: "var(--radius)", fontSize: 12, color: "var(--text3)", lineHeight: 1.7 }}>
                <strong style={{ color: "var(--text2)", display: "block", marginBottom: 4 }}>How to get the Storefront API token:</strong>
                1. Go to Shopify Admin → Settings → Apps<br/>
                2. Click "Develop apps" → Create an app<br/>
                3. Under "API credentials", enable Storefront API<br/>
                4. Add scope: <code style={{ color: "var(--accent2)" }}>unauthenticated_read_product_listings</code><br/>
      ...
      {tab === "logs" && (
        <div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {logs.length === 0 && (
              <div style={{ textAlign: "center", padding: "60px 0", color: "var(--text3)", fontSize: 14 }}>No sync logs yet. Run a sync to see results here.</div>
            )}
            {logs.map((log) => (
              <div key={log.id} style={{
                ...
                <div style={{
                  ...
                  background: log.status === "success" ? "var(--green)" : "var(--red)",
                }} />
