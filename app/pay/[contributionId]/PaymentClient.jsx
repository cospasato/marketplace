...
export default function PaymentClient({ contributionId }) {
  ...
  if (loading) return <div style={{ padding: 80, textAlign: "center", color: "var(--text3)" }}>Loading payment details...</div>;

  if (!contribution) return (
    <div style={{ padding: 80, textAlign: "center" }}>
      <p style={{ color: "var(--red)", marginBottom: 16 }}>Payment not found or already processed.</p>
      <Link href="/registry" style={{ color: "var(--accent)" }}>← Back to registries</Link>
    </div>
  );
  ...
  if (success) return (
    <div style={{ maxWidth: 520, margin: "80px auto", padding: 24, textAlign: "center" }}>
      ...
      <p style={{ fontSize: 15, color: "var(--text2)", lineHeight: 1.7, marginBottom: 24 }}>
        Thank you! We've received your payment details for <strong style={{ color: "var(--text)" }}>{contribution.item?.title}</strong>. Our team will verify your payment and arrange the gift within 24 hours.
      </p>
      <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 14, padding: "16px 20px", marginBottom: 24, textAlign: "left" }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text3)", marginBottom: 10, letterSpacing: "0.08em" }}>PAYMENT SUMMARY</div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, fontWeight: 700, color: "var(--accent)", paddingTop: 8, borderTop: "1px solid var(--border2)" }}><span>Total paid</span><span>{currency} {total.toFixed(2)}</span></div>
      </div>
      <Link href={`/registry/${contribution.registry?.slug}`} style={{ display: "inline-block", padding: "12px 28px", background: "var(--accent)", color: "var(--bg)", borderRadius: 10, fontFamily: "Georgia, serif", fontWeight: 800, fontSize: 14, textDecoration: "none" }}>
        Back to registry →
      </Link>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto", padding: "40px 24px" }}>
      <Link href={`/registry/${contribution.registry?.slug}`} style={{ fontSize: 13, color: "var(--text3)", textDecoration: "none", display: "inline-block", marginBottom: 28 }}>← Back to registry</Link>

      <h1 style={{ fontFamily: "Georgia, serif", fontSize: 26, color: "var(--text)", marginBottom: 6 }}>Complete your gift</h1>
      <p style={{ fontSize: 14, color: "var(--text2)", marginBottom: 32 }}>We'll purchase this gift on your behalf and deliver it.</p>

      {/* Gift summary */}
      <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 14, padding: "18px 20px", marginBottom: 24, display: "flex", gap: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 11, color: "var(--accent2)", marginBottom: 4, fontWeight: 700, letterSpacing: "0.08em" }}>YOU ARE GIFTING</div>
          <div style={{ fontFamily: "Georgia, serif", fontSize: 16, fontWeight: 700, color: "var(--text)", marginBottom: 4 }}>{contribution.item?.title}</div>
          <div style={{ fontSize: 12, color: "var(--text3)" }}>For {contribution.registry?.ownerName}'s {contribution.registry?.occasion || ""} registry</div>
        </div>
      </div>
      ...
      <div style={{ background: "rgba(232,213,176,0.04)", border: "1px solid rgba(232,213,176,0.12)", borderRadius: 14, padding: "18px 20px", marginBottom: 28 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "var(--accent2)", marginBottom: 12, letterSpacing: "0.08em" }}>AMOUNT TO PAY</div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, color: "var(--text2)", marginBottom: 10 }}>
          <span>Service fee <span style={{ fontSize: 11, background: "rgba(196,168,112,0.1)", color: "var(--accent2)", padding: "1px 6px", borderRadius: 4 }}>5%</span></span>
          <span>{currency} {fee.toFixed(2)}</span>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 20, fontWeight: 800, fontFamily: "Georgia, serif", color: "var(--accent)", paddingTop: 12, borderTop: "1px solid rgba(232,213,176,0.12)" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        {methods.length === 0 ? (
          <div style={{ padding: "20px", background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 10, color: "var(--text3)", fontSize: 13, textAlign: "center" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {methods.map(method => (
              <div key={method.id} onClick={() => setSelectedMethod(method)} style={{
                padding: "16px 18px", border: `1px solid ${selectedMethod?.id === method.id ? "var(--accent)" : "var(--bg5)"}`,
                borderRadius: 12, cursor: "pointer", transition: "all 0.15s",
                background: selectedMethod?.id === method.id ? "rgba(232,213,176,0.06)" : "var(--bg2)",
              }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: method.instructions ? 8 : 0 }}>
                  <div>
                    <span style={{ fontSize: 14, fontWeight: 700, color: "var(--text)" }}>{method.name}</span>
                    <span style={{ marginLeft: 10, fontSize: 11, color: "var(--text3)", background: "var(--bg3)", padding: "2px 8px", borderRadius: 4 }}>{method.type}</span>
                  </div>
                  <div style={{ width: 20, height: 20, borderRadius: "50%", border: `2px solid ${selectedMethod?.id === method.id ? "var(--accent)" : "var(--bg5)"}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {selectedMethod?.id === method.id && <div style={{ width: 10, height: 10, borderRadius: "50%", background: "var(--accent)" }} />}
                  </div>
                </div>
                ...
                {method.instructions && selectedMethod?.id === method.id && (
                  <div style={{ marginTop: 10, padding: "10px 12px", background: "var(--bg3)", borderRadius: 8, fontSize: 12, color: "var(--text2)", lineHeight: 1.7 }}>
                    📋 {method.instructions}
                  </div>
                )}
              </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 24 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 5 }}>Enter the reference number from your payment confirmation</div>
        </div>
        <div>
      ...
      <button onClick={submit} disabled={submitting} style={{ width: "100%", padding: "16px", background: submitting ? "var(--bg5)" : "var(--accent)", color: submitting ? "var(--text3)" : "var(--bg)", borderRadius: 12, border: "none", fontFamily: "Georgia, serif", fontWeight: 800, fontSize: 16, cursor: submitting ? "not-allowed" : "pointer", transition: "all 0.15s" }}>
