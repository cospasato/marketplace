  ...
  return (
    <div>
      <section style={{ padding: "100px 24px 80px", maxWidth: 1200, margin: "0 auto", textAlign: "center" }}>
        <div style={{ display: "inline-block", fontSize: 11, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--accent2)", background: "rgba(196,168,112,0.1)", border: "1px solid rgba(196,168,112,0.2)", padding: "5px 16px", borderRadius: 100, marginBottom: 28 }}>
          {stores.length} stores connected
        </div>
        <h1 style={{ fontSize: "clamp(48px, 8vw, 96px)", marginBottom: 24, color: "var(--text)" }}>
          Every store.<br /><span style={{ color: "var(--accent)" }}>One place.</span>
        </h1>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          <Link href="/products" style={{ padding: "14px 32px", background: "var(--accent)", color: "#0a0a0a", borderRadius: "var(--radius)", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 15 }}>
            Browse All Products
          </Link>
          <Link href="/search" style={{ padding: "14px 32px", background: "transparent", color: "var(--text2)", border: "1px solid var(--border2)", borderRadius: "var(--radius)", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 15 }}>
            Search
          </Link>
        </div>
      </section>
      ...
      {stores.length > 0 && (
        <section style={{ padding: "0 24px 80px", maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {stores.map((store) => (
              <Link key={store.id} href={`/store/${store.shopDomain}`} style={{ display: "flex", flexDirection: "column", gap: 10, padding: 20, background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)" }}>
                <div style={{ width: 44, height: 44, borderRadius: "var(--radius)", background: store.primaryColor || "var(--bg4)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800, fontFamily: "var(--font-display)", color: "var(--accent)" }}>
