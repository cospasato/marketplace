...
export default function ProductCard({ product, size = "md" }) {
  ...
  const handleClick = () => {
    ...
    // Use myshopify domain for the viewer route (API calls)
    // but load the productUrl which may be on the custom domain
    const viewerDomain = store.shopDomain || product.shopDomain;
    const productUrl = product.productUrl;
    const encodedUrl = encodeURIComponent(productUrl);
    const back = encodeURIComponent(window.location.pathname + window.location.search);
    router.push(`/store/${viewerDomain}?url=${encodedUrl}&back=${back}`);
  };
    ...
    >
      <div style={{ aspectRatio: "1", background: "var(--bg3)", overflow: "hidden", position: "relative" }}>
        ...
        {product.comparePrice && product.comparePrice > product.price && (
          <div style={{ position: "absolute", top: 10, left: 10, background: "var(--accent2)", color: "#1a0f00", fontSize: 11, fontWeight: 700, padding: "3px 8px", borderRadius: 6, fontFamily: "var(--font-display)", letterSpacing: "0.05em" }}>
            SALE
          </div>
        )}
      </div>
      ...
      <div style={{ padding: isLg ? "14px 16px" : "12px 14px", flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <div style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: isLg ? 15 : 13, color: "var(--text)", lineHeight: 1.3, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
          {product.title}
        </div>
