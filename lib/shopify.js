...
export function createStorefrontClient(shopDomain, storefrontToken) {
  ...
  // Headless app private tokens use Shopify-Storefront-Private-Token header
  // Custom app shpat_ tokens use X-Shopify-Storefront-Access-Token header
  // We send BOTH headers so either token type works automatically
  const headers = {
    "Content-Type": "application/json",
    "X-Shopify-Storefront-Access-Token": cleanToken,
    "Shopify-Storefront-Private-Token": cleanToken,
  };
    ...
    async query(graphql, variables = {}) {
      ...
      if (res.status === 401 || res.status === 403) {
        throw new Error(
          ...
          `Make sure you are using the PRIVATE access token from the Headless app ` +
          `(not the public token). Find it in Shopify Admin → Headless → your storefront. ` +
